...
GameSoundChanger is a retail WoW addon that maps your own player spell casts to custom sounds.

This addon was created with vibe coding.

## Beta Notes

Version `0.1.3-beta` disables buff tracking for now because recent combat/aura API restrictions can make buff-based sound alerts unreliable. This beta focuses on player spell cast alerts, clearer UI contrast, a wider editor window, profile sharing, and English/Korean UI switching.
...
4. Keep `Track: Spell` selected.
5. Cast the spell, talent, or ability you want to track.
6. Press `Use Last`.
...
The addon can play a sound when a spell starts, succeeds, or begins channeling.
...
GameSoundChanger는 월드 오브 워크래프트 리테일용 애드온입니다. 플레이어 본인의 주문 시전을 감지해서 원하는 커스텀 효과음을 재생할 수 있습니다.

이 애드온은 바이브 코딩으로 작성되었습니다.

## 베타 참고

`0.1.3-beta` 버전에서는 최근 전투/오라 API 제한으로 인해 버프 기반 사운드 알림이 불안정할 수 있어 버프 추적을 일단 비활성화했습니다. 이 베타는 플레이어 주문 시전 알림, 더 선명한 UI 대비, 더 넓은 편집 창, 프로파일 공유, 영어/한국어 UI 전환에 집중합니다.
...
4. `Track: Spell` 상태를 유지합니다.
5. 추적하고 싶은 주문, 특성, 능력을 사용합니다.
6. `Use Last` 버튼을 눌러 마지막으로 감지된 주문 ID를 불러옵니다.
...
이 애드온은 주문 시전 시작, 성공, 채널링 시작 시점에 사운드를 재생할 수 있습니다.
