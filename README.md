...
GameSoundChanger is a retail WoW addon that maps player spell casts or player buff activations to custom sounds.
...
4. Cast the spell, use the talent or ability, or gain the buff you want to track.
5. Press `Use Last`.
...
7. Press `Preview` to test the sound.
8. Press `Add/Update` to save the rule.
...
GameSoundChanger는 월드 오브 워크래프트 리테일용 애드온입니다. 플레이어의 주문 시전이나 버프 발동을 감지해서 원하는 커스텀 효과음을 재생할 수 있습니다.
...
- 플레이어 주문 시전을 추적합니다.
- 플레이어 버프 적용 또는 갱신을 추적합니다.
- 마지막으로 감지된 주문 또는 버프를 빠르게 등록할 수 있습니다.
...
7. `Preview`로 미리 들어봅니다.
8. `Add/Update`를 눌러 규칙을 저장합니다.
